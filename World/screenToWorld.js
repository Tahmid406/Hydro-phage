...
function handleArrowKeyPanning() {
  let dx = 0,
    dy = 0;
  if (arrowKeys.left) dx -= arrowPanSpeed / zoom;
  if (arrowKeys.right) dx += arrowPanSpeed / zoom;
  if (arrowKeys.up) dy -= arrowPanSpeed / zoom;
  if (arrowKeys.down) dy += arrowPanSpeed / zoom;

  if (dx !== 0 || dy !== 0) {
    offset.x += dx;
    offset.y += dy;

    movedSinceLastUpdate += Math.sqrt(dx * dx + dy * dy);
    if (movedSinceLastUpdate >= 50) {
      updateGridVisibility();
      movedSinceLastUpdate = 0;
    }
  }
}
